//***********special thanks to Dr. Martin Buttler**************
using System;
...
namespace PCBDriller
{
    class optimumPath
    {
        ...
        private static ToolStripProgressBar bar;
        private static ToolStripLabel lbl;
        public static BackgroundWorker worker_point_analyzer;
        private static bool nnh = true;
        private static wait w;

        public static ArrayList travelling_Salesman(ArrayList list, bool use_Nearest_Neighbour_Heristics, ToolStripProgressBar b, ToolStripLabel l)
        {
            ...
            return getTheSortedPoints(list);
        }
        private static ArrayList getTheSortedPoints(ArrayList list)
        {
            worker_point_analyzer = new BackgroundWorker();
            worker_point_analyzer.WorkerReportsProgress = true;
            worker_point_analyzer.WorkerSupportsCancellation = true;
            worker_point_analyzer.DoWork += new DoWorkEventHandler(worker_nearest_insertion_DoWork);
            worker_point_analyzer.ProgressChanged += new ProgressChangedEventHandler(worker_nearest_insertion_ProgressChanged);
            worker_point_analyzer.RunWorkerCompleted += new RunWorkerCompletedEventHandler(worker_nearest_insertion_RunWorkerCompleted);
            worker_point_analyzer.RunWorkerAsync(list);
        ...
        static void worker_nearest_insertion_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            bar.Value = 0;
            w.Close();
            lbl.Text = "Idle...";
        ...
        static void worker_nearest_insertion_ProgressChanged(object sender, ProgressChangedEventArgs e)
        {
            ...
            if (e.ProgressPercentage < 10)
                p = "0" + p;
            w.lblPerce.Text = p;
            lbl.Text = "Finding optimum path.. Completed "+e.ProgressPercentage.ToString()+" %";
        }

        static void worker_nearest_insertion_DoWork(object sender, DoWorkEventArgs e)
        {
            ArrayList list = new ArrayList((ArrayList)e.Argument);
            ...
            if (!nnh)
            {
                for (int i = 0; i < list.Count-1; i++)
                {
                    ...
                    worker_point_analyzer.ReportProgress((int)progress);
                }
            }
            else
            {
